...
app.get('/', async (req, res) => {
    ...
    //const newUser = await Musica.find({ title: { $regex: re, $options: 'i' } });
    //console.log('sibsuiadauygc',newUser)
    //const newUser2 = await Musica.find({ album_artist: { $regex:re, $options: 'i' } });

    const iii = await Musica.find({ $or:[ {title: { $regex: re, $options: 'i' }}, {album_artist: { $regex:re, $options: 'i' } }]});
    //console.log('es',iii)
    //res.send(salida);
...
app.post('/year', async (req, res) => {
    ...
    const re = new RegExp(se);
    const iii = await Musica.find({ year: { $regex: re, $options: 'i' }});
    console.log('es',iii)
    //res.send(salida);
    res.render('index',{peoples:iii,se});
});
